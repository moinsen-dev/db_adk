    ...
    try:
        ...
        for prop in required_properties:
            if prop not in schema:
                raise ValueError(f"Tool parameters schema missing required property: {prop}")
...
def validate_agent_config(config):
    ...
    if config["agent_type"] == "LLM":
        if "prompt_template" not in config:
            raise ValueError("LLM agent configuration missing required field: prompt_template")
